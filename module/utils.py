...
def evaluate_model(data_iter, net, loss, device = device):
    ...
    with torch.no_grad():
        net.eval() 
        for X, adjs, y in data_iter:
            X = X.to(device)
            y_hat = net(X, adjs).to(device)
            y = y.to(device)
...
def train_model(train_iter, valid_iter, net, loss, optimizer, lamb = 0.001, max_epochs = 100, adjust_lr = None, 
    ...
    for epoch in range(max_epochs):
        ...
        for X, adjs, y in tqdm(train_iter, ncols = 50):
            X = X.to(device)
            adjs = adjs.to(device)
            y_hat, L_reg = net(X, adjs)
            y = y.to(device)
        ...
        print('epoch %d / %d, train loss %.4f (acc %.6f), valid loss %.4f (acc %.6f)'
              % (epoch+1, max_epochs, train_loss, train_acc, valid_loss, valid_acc), flush=True)
